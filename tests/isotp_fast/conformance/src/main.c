...
static void send_sf(void)
{
    ...
    ret = isotp_fast_send(&ctx, random_data, DATA_SIZE_SF, tx_addr, NULL);
...
static void send_test_data(const uint8_t *data, size_t len)
{
    ...
    ret = isotp_fast_send(&ctx, data, len, tx_addr, INT_TO_POINTER(ISOTP_N_OK));
    zassert_equal(ret, 0, "Send returned %d", ret);
}
...
ZTEST(isotp_fast_conformance, test_receive_sf)
{
    ...
    send_frame_series(&single_frame, 1, rx_can_id);
...
ZTEST(isotp_fast_conformance, test_send_sf_fixed)
{
    ...
    ret = isotp_fast_send(&ctx, random_data, DATA_SIZE_SF, tx_addr, INT_TO_POINTER(ISOTP_N_OK));
    zassert_equal(ret, 0, "Send returned %d", ret);
...
ZTEST(isotp_fast_conformance, test_receive_sf_fixed)
{
    ...
    send_frame_series(&single_frame, 1, rx_can_id | 0xFF);
    get_sf(&ctx, DATA_SIZE_SF);

    /* different priority */
    send_frame_series(&single_frame, 1, rx_can_id | (7U << 26));
    get_sf(&ctx, DATA_SIZE_SF);

    /* different target address (should fail) */
    send_frame_series(&single_frame, 1, rx_can_id | 0xFF00);
    get_sf_ignore(&ctx);
}
...
ZTEST(isotp_fast_conformance, test_receive_data)
{
    ...
    send_frame_series(&ff_frame, 1, rx_can_id);

    check_frame_series(&fc_frame, 1, &frame_msgq);

    send_frame_series(des_frames, ARRAY_SIZE(des_frames), rx_can_id);
...
ZTEST(isotp_fast_conformance, test_receive_data_blocks)
{
    ...
    while (remaining_frames) {
        ...
        if (remaining_frames >= fc_opts.bs) {
            send_frame_series(data_frame_ptr, fc_opts.bs, rx_can_id);
            data_frame_ptr += fc_opts.bs;
            remaining_frames -= fc_opts.bs;
        }
        else {
            send_frame_series(data_frame_ptr, remaining_frames, rx_can_id);
...
ZTEST(isotp_fast_conformance, test_send_timeouts)
{
    ...
    isotp_fast_send(&ctx, random_data, sizeof(random_data), tx_addr,
    ...
    k_sem_reset(&send_compl_sem);
    ret = isotp_fast_send(&ctx, random_data, sizeof(random_data), tx_addr,
                          INT_TO_POINTER(ISOTP_N_TIMEOUT_BS));
    zassert_equal(ret, ISOTP_N_OK, "Send returned %d", ret);

    send_frame_series(&fc_cts_frame, 1, rx_can_id);
...
ZTEST(isotp_fast_conformance, test_stmin)
{
    ...
    filter_id = add_rx_msgq(rx_can_id, CAN_EXT_ID_MASK);
    zassert_true((filter_id >= 0), "Negative filter number [%d]", filter_id);
    ...
    send_frame_series(&fc_frame, 1, tx_can_id);
...
ZTEST(isotp_fast_conformance, test_sender_fc_errors)
{
    ...
    k_sem_reset(&send_compl_sem);
    ret = isotp_fast_send(&ctx, random_data, DATA_SEND_LENGTH, tx_addr,
    ...
    ret = isotp_fast_send(&ctx, random_data, 5 * 1024, tx_addr, NULL);
...
#ifdef CONFIG_CAN_FD_MODE
ZTEST(isotp_fast_conformance, test_sf_length)
{
    int ret;
    struct frame_desired des_frame;
    ...
    ret = isotp_fast_send(&ctx, random_data, 7, tx_addr, INT_TO_POINTER(ISOTP_N_OK));
    zassert_equal(ret, 0, "Send returned %d", ret);
    ...
    ret = isotp_fast_send(&ctx, random_data, 9, tx_addr, INT_TO_POINTER(ISOTP_N_OK));
    zassert_equal(ret, 0, "Send returned %d", ret);
...
ZTEST(isotp_fast_conformance, test_send_length_63)
{
    ...
    ret = isotp_fast_send(&ctx, random_data, 63, tx_addr, INT_TO_POINTER(ISOTP_N_OK));
    zassert_equal(ret, 0, "Send returned %d", ret);

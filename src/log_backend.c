...
THINGSET_ADD_GROUP(TS_ID_ROOT, TS_ID_LOG, "Log", THINGSET_NO_CALLBACK);

#ifdef CONFIG_THINGSET_LOG_BACKEND_USE_UPTIME
THINGSET_ADD_ITEM_UINT32(TS_ID_LOG, TS_ID_LOG_TIME, "t_s", &log_timestamp, THINGSET_ANY_R, 0);
#endif

THINGSET_ADD_ITEM_STRING(TS_ID_LOG, TS_ID_LOG_MESSAGE, "rMessage", log_msg, sizeof(log_msg),
                         THINGSET_ANY_R, 0);

THINGSET_ADD_ITEM_STRING(TS_ID_LOG, TS_ID_LOG_MODULE, "oModule", log_module, sizeof(log_module),
                         THINGSET_ANY_R, 0);

THINGSET_ADD_ITEM_UINT8(TS_ID_LOG, TS_ID_LOG_LEVEL, "oLevel", &log_level, THINGSET_ANY_R, 0);

THINGSET_ADD_GROUP(TS_ID_REPORTING, TS_ID_REP_LOG, "Log", THINGSET_NO_CALLBACK);

THINGSET_ADD_GROUP(TS_ID_REP_LOG, TS_ID_REP_LOG_SELF, "_", THINGSET_NO_CALLBACK);

THINGSET_ADD_ITEM_BOOL(TS_ID_REP_LOG_SELF, TS_ID_REP_LOG_ENABLE, "sEnable", &pub_logs_enable,
                       THINGSET_ANY_RW, TS_SUBSET_NVM);

THINGSET_ADD_ITEM_UINT8(TS_ID_REP_LOG_SELF, TS_ID_REP_LOG_MAX_LEVEL, "sMaxLevel", &max_level,
                        THINGSET_ANY_RW, TS_SUBSET_NVM);
